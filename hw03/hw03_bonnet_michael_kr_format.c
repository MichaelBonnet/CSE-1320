...
int main(void) { // the main function executes all other functions to get desired output.
    ...
    do { // do-while loop makes sure the user is prompted for input at least once and is promtped as long as they don't type the exit condition
        ...
        if (input == -1) { // checks to see if the user wants to exit the program, done by typing -1 for input
            ...
            for (int i = 0; i <= 28; i+=4) { // loops through each bit starting from the rightmost
                digit_storage[7 - (i / 4)] = ((input & bitmask(i)) >> i); // evaluates the isolated 4 bits to get a packed BCD digit and stores it in an array                                                                         // as bit evaluation starts from the right, the array is populated backwards, from a[8] to a[0]
            }
